...
class Entertainment(commands.Cog):
    ...
    async def hot_chocolate(self, ctx: commands.Context, user: discord.Member = None, *, reason: str = ""):
        ...
        if not user or user.id == ctx.author.id:
            async with ctx.typing():
                return await general.send(langs.gls("fun_hc_self", locale, ctx.author.name), ctx.channel, file=discord.File("assets/hc.gif", "chocolate.gif"))
        if user.id == self.bot.user.id:
            return await general.send(langs.gls("fun_hc_me", locale), ctx.channel)
        if user.bot:
            return await general.send(langs.gls("fun_hc_bot", locale), ctx.channel)
        beer_offer = langs.gls("fun_beer_offer", locale, user.name, ctx.author.name, "☕🍫")
        if reason:
            beer_offer += langs.gls("fun_beer_reason", locale, reason)
        msg = await general.send(beer_offer, ctx.channel)
        try:
            ...
            await msg.delete()
            return await general.send(langs.gls("fun_hc_timeout", locale, user.name, ctx.author.name), ctx.channel)
        except discord.Forbidden:
            beer = langs.gls("fun_beer_no_react", locale, user.name, ctx.author.name, "☕🍫")
            if reason:
                beer += reason
