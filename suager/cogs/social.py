...
def give(u1: str, u2: str, emote: str) -> str:
...
class Social(commands.Cog):
    ...
    def data_update(self, uid_give: int, uid_receive: int, gid: int, key_g: str, key_r: str, in_g: int, in_r: int):
        ...
        if not data_giver:
            ...
            self.db.execute(f"UPDATE counters SET {key_g}=? WHERE uid=? AND gid=?", (nu, uid_give, gid))
        if not data_receive:
            data = self.empty.copy()
            data[0] = uid_receive
            data[1] = gid
            ...
            self.db.execute(f"UPDATE counters SET {key_r}=? WHERE uid=? AND gid=?", (number, uid_receive, gid))
        return number
    ...
    def data_update2(self, uid: int, gid: int, key: str, ind: int):
        ...
        if not data_giver:
            ...
            n = data_giver[key]
            number = 1 if n is None else n + 1
            self.db.execute(f"UPDATE counters SET {key}=? WHERE uid=? AND gid=?", (number, uid, gid))
        # return number
    ...
    async def pat(self, ctx: commands.Context, user: discord.Member):
        ...
        if ctx.author == user:
            return await general.send(f"Don't be like that, {ctx.author.name}", ctx.channel)
        ...
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "pats_given", "pats_received", 16, 17)
    ...
    async def hug(self, ctx: commands.Context, user: discord.Member):
        ...
        if ctx.author == user:
            return await general.send("Alone?", ctx.channel, embed=discord.Embed(colour=general.random_colour()).set_image(
                url="https://cdn.discordapp.com/attachments/610482988123422750/673641089218904065/selfhug.gif"))
        if user.id == self.bot.user.id:
            return await general.send(f"*Hugs {ctx.author.name} back* {emotes.AlexHeart}", ctx.channel)
        if user.bot:
            return await general.send(f"I don't think {user.name} is going to respond...", ctx.channel)
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "hugs_given", "hugs_received", 10, 11)
        embed.title = f"**{user.name}** just got a hug from **{ctx.author.name}**"
    ...
    async def cuddle(self, ctx: commands.Context, user: discord.Member):
        ...
        if user.id == self.bot.user.id:
            return await general.send(f"*Cuddles {ctx.author.name} back* {emotes.AlexHeart}", ctx.channel)
        if user.bot:
            return await general.send(f"I don't think {user.name} is going to respond...", ctx.channel)
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "cuddles_given", "cuddles_received", 6, 7)
    ...
    async def lick(self, ctx: commands.Context, user: discord.Member):
        ...
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "licks_given", "licks_received", 14, 15)
    ...
    async def kiss(self, ctx: commands.Context, user: discord.Member):
        ...
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "kisses_given", "kisses_received", 12, 13)
    ...
    async def bite(self, ctx: commands.Context, user: discord.Member):
        ...
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "bites_given", "bites_received", 4, 5)
    ...
    async def smell(self, ctx: commands.Context, user: discord.Member):
        ...
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "sniffs_given", "sniffs_received", 20, 21)
    ...
    async def highfive(self, ctx: commands.Context, user: discord.Member):
        ...
        if user.bot:
            return await general.send(f"I don't think {user.name} is going to respond...", ctx.channel)
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "high_fives_given", "high_fives_received", 8, 9)
    ...
    async def poke(self, ctx: commands.Context, user: discord.Member):
        ...
        if user.bot:
            return await general.send(f"I don't think {user.name} is going to respond...", ctx.channel)
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "pokes_given", "pokes_received", 42, 43)
    ...
    async def boop(self, ctx: commands.Context, user: discord.Member):
        ...
        if user.bot:
            return await general.send(f"I don't think {user.name} is going to respond...", ctx.channel)
        embed = discord.Embed(colour=general.random_colour())
        self.data_update(ctx.author.id, user.id, ctx.guild.id, "boops_given", "boops_received", 44, 45)
    ...
    async def ship(self, ctx: commands.Context, user1: discord.Member, user2: discord.Member):
        ...
        if user1.id == self.bot.user.id or user2.id == self.bot.user.id:
            if user1.id != 302851022790066185 and user2.id != 302851022790066185:
                return await general.send("No thanks.", ctx.channel)
            pr = True
    ...
    async def sleepy(self, ctx):
        ...
        self.data_update2(ctx.author.id, ctx.guild.id, "sleepy", 32)
        return await general.send(None, ctx.channel, embed=embed)
    ...
    async def cry(self, ctx):
        ...
        self.data_update2(ctx.author.id, ctx.guild.id, "cried", 31)
        return await general.send(None, ctx.channel, embed=embed)
    ...
    async def blush(self, ctx):
        ...
        self.data_update2(ctx.author.id, ctx.guild.id, "blushed", 30)
        return await general.send(None, ctx.channel, embed=embed)
    ...
    async def smile(self, ctx):
        ...
        self.data_update2(ctx.author.id, ctx.guild.id, "smiled", 33)
        return await general.send(None, ctx.channel, embed=embed)
    ...
    async def fuck(self, ctx, user: discord.Member):
        ...
        if user == ctx.author:
            return await general.send(f"You what {emotes.UmmOK}", ctx.channel)
    ...
    async def bean(self, ctx: commands.Context, user: discord.Member):
        """ Bean someone """
        if user == ctx.author:
            return await general.send("How are you planning to do that?", ctx.channel)
        if user.id == 302851022790066185:
            return await general.send("You're not allowed to do that.", ctx.channel)
        if user.id == self.bot.user.id:
            return await general.send("You can't do that", ctx.channel)
    ...
    async def trash(self, ctx, user: discord.Member):
        ...
        if bio is None:
            return await general.send("An error occured, and the image was not created...", ctx.channel)
    ...
    async def cookie(self, ctx: commands.Context, user: discord.Member):
        ...
        if user == ctx.author:
            return await general.send(emotes.AlexPat, ctx.channel)
        if user.id == self.bot.user.id:
            return await general.send("Thanks, but I can't eat normal food...", ctx.channel)
        if user.bot:
            return await general.send("Bots can't eat normal food...", ctx.channel)
        output = give(ctx.author.name, user.name, "🍪")
    ...
    async def fruit_snacks(self, ctx: commands.Context, user: discord.Member):
        ...
        if user.bot:
            return await general.send("Bots can't eat normal food...", ctx.channel)
        output = give(ctx.author.name, user.name, random.choice(list("🍏🍎🍐🍊🍌🍉🍇🍓🍒🍍")))
    ...
    async def top_counters(self, ctx):
        """ Top counters """
        embed = discord.Embed(colour=general.random_colour())
        embed.title = f"Spammer leaderboard for {ctx.guild.name}"
