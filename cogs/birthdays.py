...
from utils import general, languages, permissions
...
class Birthdays(commands.Cog):
    ...
    async def birthday(self, ctx: commands.Context, *, user: discord.User = None):
        ...
        if ctx.invoked_subcommand is None:
            ...
            if user.id == self.bot.user.id:
                return await general.send(languages.gls(self.birthday_self, locale), ctx.channel)
            has_birthday = self.check_birthday(user.id)
            if not has_birthday:
                return await general.send(languages.gls("birthdays_birthday_not_saved", locale, user.name), ctx.channel)
            birthday = languages.gts_date(has_birthday, locale, False, False)
            if user == ctx.author:
                return await general.send(languages.gls("birthdays_birthday_your", locale, birthday), ctx.channel)
            return await general.send(languages.gls("birthdays_birthday_general", locale, str(user), birthday), ctx.channel)
    ...
    async def set(self, ctx: commands.Context, date: str):
        ...
        if re.compile(self.re_timestamp).search(date):
            try:
                ...
                return await general.send(languages.gls("birthdays_set_invalid", locale), ctx.channel)
        else:
            return await general.send(languages.gls("birthdays_set_invalid", locale), ctx.channel)
        date = languages.gts_date(timestamp, locale, False, False)
        ...
        if has_birthday:
            ...
            old_date = languages.gts_date(has_birthday, locale, False, False)
            return await general.send(languages.gls("birthdays_set_already", locale, ctx.author.name, date, old_date), ctx.channel)
        else:
            self.bot.db.execute(f"INSERT INTO {self.birthdays} VALUES (?, ?, ?)", (ctx.author.id, timestamp, False))
            return await general.send(languages.gls("birthdays_set_set", locale, ctx.author.name, date), ctx.channel)
    ...
    async def delete(self, ctx: commands.Context):
        ...
        return await general.send(languages.gls("birthdays_clear", locale, ctx.author.name), ctx.channel)
