...
from utils import arg_parser, bases, emotes, general, http, languages, permissions, time, times
...
async def time_diff(ctx: commands.Context, string: str, multiplier: int):
    ...
    if errors:
        return await general.send(languages.gls("util_timediff_error", locale, then), ctx.channel)
        # return await general.send(f"Error converting time difference: {then}", ctx.channel)
    difference = languages.td_rd(delta, locale, accuracy=7, brief=False, suffix=True)
    time_now = languages.gts(time.now(None), locale, True, False, False, True, False)
    time_then = languages.gts(then, locale, True, False, False, True, False)
    return await general.send(languages.gls("util_timediff", locale, time_now, difference, time_then), ctx.channel)
...
class Utility(commands.Cog):
    ...
    async def current_time(self, ctx: commands.Context):
        ...
        if locale in ["rsl-1k", "rsl-1i"]:
            a = times.time_kargadia(time.now(None)).str(dow=True, era=False, month=False)
            b = languages.gts(time.now(None), locale, True, False, True, True, False)
            d = languages.gts(time.now_k(), locale, True, False, True, True, False)
            send += f"Zymlä: **{b}**\n" \
                    f"Senkadar Laikadu: **{d}**\n" \
                    f"Kargadia: **{a}**"
        else:
            send += languages.gls("util_time_bot", locale, languages.gts(time.now(self.bot.local_config["timezone"]), locale, True, False, True, True, False))
            send += f"UTC/GMT: **{languages.gts(time.now(None), locale, True, False, True, True, False)}**"
            if ctx.guild is not None and ctx.guild.id in [568148147457490954, 738425418637639775]:
                send += f"\nSenko Lair: **{languages.gts(time.now_sl(), locale, True, False, True, True, False)}**"  # \n" \
        ...
        if data:
            send += languages.gls("util_time_custom", locale, languages.gts(time.set_tz(time.now(None), data['tz']), locale, True, False, True, True, False))
    ...
    async def set_timezone(self, ctx: commands.Context, tz: str = None):
        ...
        if tz is None:
            ...
            return await general.send(languages.gls("util_time_tz_reset", locale), ctx.channel)
        try:
            data = self.bot.db.fetchrow("SELECT * FROM timezones WHERE uid=?", (ctx.author.id,))
            ...
            return await general.send(languages.gls("util_time_tz", locale, _tz), ctx.channel)
            ...
            return await general.send(languages.gls("util_time_tz_error", locale, tz), ctx.channel, file=file)
    ...
    async def time_since(self, ctx: commands.Context, year: int = None, month: int = 1, day: int = 1, hour: int = 0, minute: int = 0, second: int = 0):
        ...
        try:
            ...
            difference = languages.td_dt(date, locale, accuracy=7, brief=False, suffix=True)  # time.human_timedelta(date, accuracy=7)
            current_time = languages.gts(now, locale, True, False, True, True, True)  # time.time_output(now, True, True, True)
            specified_time = languages.gts(date, locale, True, False, True, True, True)  # time.time_output(date, True, True, True)
            return await general.send(languages.gls("util_timesince", locale, current_time, specified_time, difference), ctx.channel)
        except Exception as e:
            return await general.send(languages.gls("util_timesince_error", locale, type(e).__name__, str(e)), ctx.channel)
    ...
    async def weather(self, ctx: commands.Context, *, place: str):
        ...
        try:
            ...
            if code == 200:
                ...
                if country:
                    country_name = languages.gls(f"z_data_country_{country}", locale)
                    emote = f":flag_{country}: "
                else:
                    country_name = "Not a country"
                    emote = ""
                embed.title = languages.gls("util_weather_title", locale, data['name'], country_name, emote=emote)
                embed.description = languages.gls("util_weather_desc", locale, local_time)
                weather_icon = data['weather'][0]['icon']
                embed.set_thumbnail(url=f"http://openweathermap.org/img/wn/{weather_icon}@2x.png")
                embed.add_field(name=languages.gls("util_weather_weather", locale), value=data['weather'][0]['description'].capitalize(), inline=True)
                _tk = data['main']['temp']
                _tc = _tk - 273.15
                _tf = _tc * 1.8 + 32
                tk, tc, tf = languages.gfs(_tk, locale, 1), languages.gfs(_tc, locale, 1), languages.gfs(_tf, locale, 1)
                embed.add_field(name=languages.gls("util_weather_temperature", locale), value=f"**{tc}°C** | {tk} K | {tf}°F", inline=True)
                embed.add_field(name=languages.gls("util_weather_pressure", locale), value=f"{languages.gns(data['main']['pressure'], locale)} hPa", inline=True)
                embed.add_field(name=languages.gls("util_weather_humidity", locale), value=languages.gfs(data['main']['humidity'] / 100, locale, 0, True), inline=True)
                _sm = data['wind']['speed']
                _sk = _sm * 3.6
                _sb = _sk / 1.609  # imperial system bad
                sm, sk, sb = languages.gfs(_sm, locale, 2), languages.gfs(_sk, locale, 2), languages.gfs(_sb, locale, 2)
                embed.add_field(name=languages.gls("util_weather_wind", locale), value=languages.gls("util_weather_wind_data", locale, sm, sk, sb), inline=True)
                embed.add_field(name=languages.gls("util_weather_clouds", locale), value=languages.gfs(data['clouds']['all'] / 100, locale, 0, True), inline=True)
                sr = data['sys']['sunrise']
                ss = data['sys']['sunset']
                now = time.now(None)
                ...
                if sr != 0 and ss != 0:
                    ...
                    sr, tr = languages.gts(srt, _time_locale, False, seconds=False), languages.td_dt(srt, locale, source=now_l, accuracy=1, suffix=True)
                    ss, ts = languages.gts(sst, _time_locale, False, seconds=False), languages.td_dt(sst, locale, source=now_l, accuracy=1, suffix=True)
                    embed.add_field(name=languages.gls("util_weather_sunrise", locale), value=f"{sr} | {tr}", inline=True)
                    embed.add_field(name=languages.gls("util_weather_sunset", locale), value=f"{ss} | {ts}", inline=True)
                embed.timestamp = now
            else:
                return await general.send(languages.gls("util_weather_error", locale, place, code, data["message"]), ctx.channel)
        except Exception as e:
            return await general.send(languages.gls("util_weather_error", locale, place, type(e).__name__, str(e)), ctx.channel)
    ...
    async def colour(self, ctx: commands.Context, colour: str = "random"):
        ...
        if colour.lower() == "random":
            ...
            try:
                ...
                if a == 3:
                    ...
                    a = 6
                if a != 6:
                    return await general.send(languages.gls("images_colour_invalid_value", locale), ctx.channel)
                _colour = f"{int(colour, 16):06x}"
            except Exception as e:
                return await general.send(languages.gls("images_colour_invalid", locale, type(e).__name__, str(e)), ctx.channel)
        hex_6 = _colour[:6]
        ...
        embed.add_field(name=languages.gls("images_colour_hex", locale), value=f'#{hex_6}', inline=False)
        embed.add_field(name=languages.gls("images_colour_int", locale), value=str(int_6), inline=False)
        embed.add_field(name=languages.gls("images_colour_rgb", locale) + " (0-255)", value=str(rgba_255), inline=False)
        embed.add_field(name=languages.gls("images_colour_rgb", locale) + " (0-1)", value=str(rgba_1).replace("'", ""), inline=False)
        embed.add_field(name=languages.gls("images_colour_brightness", locale), value=str(brightness), inline=False)
        embed.add_field(name=languages.gls("images_colour_font", locale), inline=False, value="#000000" if brightness >= 128 or green == 255 else "#ffffff")
    ...
    async def roll(self, ctx: commands.Context, num1: int = 6, num2: int = 1):
        ...
        n1, n2, no = languages.gns(v1, locale), languages.gns(v2, locale), languages.gns(r, locale)
        return await general.send(languages.gls("fun_roll", locale, ctx.author.name, n1, n2, no), ctx.channel)
    ...
    async def tell(self, ctx: commands.Context, channel: discord.TextChannel, *, message: str):
        ...
        try:
            ...
            await general.send(languages.gls("fun_say_delete_fail", locale, type(e).__name__, str(e)), ctx.channel, delete_after=5)
        if channel.guild != ctx.guild:
            return await general.send(languages.gls("fun_tell_guilds", locale), ctx.channel)
        try:
            await general.send(message, channel, u=True, r=True)
        except Exception as e:
            return await general.send(languages.gls("fun_tell_fail", locale, type(e).__name__, str(e)), ctx.channel)
        return await general.send(languages.gls("fun_tell_success", locale, channel.mention), ctx.channel, delete_after=5)
    ...
    async def say(self, ctx: commands.Context, *, message: str):
        ...
        return await general.send(languages.gls("fun_say_success", locale), ctx.channel, delete_after=5)
    ...
    async def vote(self, ctx: commands.Context, *, question: str):
        """ Start a vote """
        locale = languages.gl(ctx)
        message = await general.send(languages.gls("fun_vote", locale, ctx.author.name, languages.gls(f"fun_vote_{str(ctx.invoked_with).lower()}", locale), question),
                                     ctx.channel)
    ...
    async def avatar(self, ctx: commands.Context, *, who: discord.User = None):
        ...
        return await general.send(languages.gls("discord_avatar", languages.gl(ctx), user.name, user.avatar_url_as(size=1024, static_format='png')), ctx.channel)
        ...
        if ctx.invoked_subcommand is None:
            ...
            if role is None:
                ...
                for num, role in enumerate(sorted(ctx.guild.roles, reverse=True), start=1):
                    all_roles += languages.gls("discord_role_list_item", locale, languages.gns(num, locale, 2, False), languages.gns(len(role.members)), role=role)
                data = BytesIO(all_roles.encode('utf-8'))
                return await general.send(languages.gls("discord_role_list", locale, ctx.guild.name), ctx.channel,
                                          file=discord.File(data, filename=f"{time.file_ts('Roles')}"))
            else:
                embed = discord.Embed(colour=role.colour)
                embed.title = languages.gls("discord_role_about", locale, role.name)
                embed.set_thumbnail(url=ctx.guild.icon_url_as(size=1024))
                embed.add_field(name=languages.gls("discord_role_name", locale), value=role.name, inline=True)
                embed.add_field(name=languages.gls("discord_role_id", locale), value=str(role.id), inline=True)
                embed.add_field(name=languages.gls("generic_members", locale), value=f"{len(role.members):,}", inline=True)
                embed.add_field(name=languages.gls("discord_role_colour", locale), value=str(role.colour), inline=True)
                embed.add_field(name=languages.gls("discord_role_mentionable", locale), value=languages.yes(role.mentionable, locale), inline=True)
                embed.add_field(name=languages.gls("discord_role_hoisted", locale), value=languages.yes(role.hoist, locale), inline=True)
                embed.add_field(name=languages.gls("discord_role_position", locale), value=languages.gns(role.position, locale), inline=True)
                embed.add_field(name=languages.gls("generic_created_at", locale), value=languages.gts(role.created_at, locale, short=False), inline=True)
                embed.add_field(name=languages.gls("discord_role_default", locale), value=languages.yes(role.is_default(), locale), inline=True)
                return await general.send(None, ctx.channel, embed=embed)
    ...
    async def role_members(self, ctx: commands.Context, *, role: discord.Role):
        ...
        for i in range(len(members)):
            m += f"[{languages.gns(i + 1, locale, 2, False)}] {members[i]}\n"
        rl = len(m)
        send = languages.gls("discord_role_members", locale, ctx.guild.name)
        if rl > 1900:
    ...
    async def joined_at(self, ctx: commands.Context, *, who: discord.Member = None):
        ...
        user = who or ctx.author
        locale = languages.gl(ctx)
        return await general.send(languages.gls("discord_joined_at", locale, user, ctx.guild.name, languages.gts(user.joined_at, locale, short=False)), ctx.channel)
    ...
    async def created_at(self, ctx: commands.Context, *, who: discord.User = None):
        ...
        user = who or ctx.author
        locale = languages.gl(ctx)
        return await general.send(languages.gls("discord_created_at", locale, user, languages.gts(user.created_at, locale, short=False)), ctx.channel)
    ...
    async def user(self, ctx: commands.Context, *, who: discord.Member = None):
        ...
        embed.title = languages.gls("discord_user_about", locale, user.name)
        embed.set_thumbnail(url=user.avatar_url_as(size=1024))
        embed.add_field(name=languages.gls("discord_user_username", locale), value=user, inline=True)
        embed.add_field(name=languages.gls("discord_user_nickname", locale), value=user.nick, inline=True)
        embed.add_field(name=languages.gls("discord_user_id", locale), value=user.id, inline=True)
        embed.add_field(name=languages.gls("generic_created_at", locale), value=languages.gts(user.created_at, locale, short=False), inline=False)
        embed.add_field(name=languages.gls("discord_user_joined_at", locale), value=languages.gts(user.joined_at, locale, short=False), inline=False)
        if len(user.roles) < 15:
            r = user.roles
            r.sort(key=lambda x: x.position, reverse=True)
            ar = [f"<@&{x.id}>" for x in r if x.id != ctx.guild.default_role.id]
            roles = ', '.join(ar) if ar else languages.gls("generic_none", locale)
            b = len(user.roles) - 1
            roles += languages.gls("discord_user_roles_overall", locale, languages.gns(b, locale))
        else:
            roles = languages.gls("discord_user_roles_many", locale, languages.gns(len(user.roles) - 1))
        embed.add_field(name=languages.gls("discord_user_roles", locale), value=roles, inline=False)
        return await general.send(None, ctx.channel, embed=embed)
    ...
    async def who_is(self, ctx: commands.Context, *, user_id: int):
        ...
        try:
            ...
            return await general.send(languages.gls("events_err_error", locale, "NotFound", str(e)), ctx.channel)
        embed = discord.Embed(colour=general.random_colour())
        embed.title = languages.gls("discord_user_about", locale, user.name)
        embed.set_thumbnail(url=user.avatar_url_as(size=1024))
        embed.add_field(name=languages.gls("discord_user_username", locale), value=user, inline=True)
        embed.add_field(name=languages.gls("discord_user_id", locale), value=user.id, inline=True)
        embed.add_field(name=languages.gls("generic_created_at", locale), value=languages.gts(user.created_at, locale, short=False), inline=True)
        return await general.send(None, ctx.channel, embed=embed)
    ...
    async def emoji(self, ctx: commands.Context, emoji: discord.Emoji):
        ...
        c = languages.gls(f"discord_emoji_{e}", locale)
        embed = discord.Embed(colour=general.random_colour())
        embed.description = languages.gls("discord_emoji", locale, c, emoji.name, emoji.id, languages.yes(emoji.animated, locale), emoji.guild.id,
                                          languages.gts(emoji.created_at, locale, short=False), emoji.url)
    ...
    async def server(self, ctx: commands.Context):
        ...
        if ctx.invoked_subcommand is None:
            ...
            embed = discord.Embed(colour=general.random_colour())
            embed.set_thumbnail(url=ctx.guild.icon_url_as(size=1024))
            embed.title = languages.gls("discord_server_about", locale, ctx.guild.name)
            embed.add_field(name=languages.gls("discord_server_name", locale), value=ctx.guild.name, inline=True)
            embed.add_field(name=languages.gls("discord_server_id", locale), value=ctx.guild.id, inline=True)
            embed.add_field(name=languages.gls("discord_server_owner", locale), inline=True, value=f"{ctx.guild.owner}\n({ctx.guild.owner.display_name})")
            embed.add_field(name=languages.gls("generic_members", locale), value=languages.gns(ctx.guild.member_count, locale), inline=True)
            embed.add_field(name=languages.gls("discord_server_bots", locale), inline=True,
                            value=f"{languages.gns(bots, locale)} ({languages.gfs(bots_amt, locale, per=True)})")
            embed.add_field(name=languages.gls("discord_server_region", locale), value=ctx.guild.region, inline=True)
            embed.add_field(name=languages.gls("discord_server_roles", locale), value=languages.gns(len(ctx.guild.roles), locale), inline=True)
            try:
                embed.add_field(name=languages.gls("discord_server_bans", locale), value=languages.gns(len(await ctx.guild.bans()), locale), inline=True)
            except discord.Forbidden:
            ...
            embed.add_field(name=languages.gls("discord_server_verification", locale), inline=True, value=str(ctx.guild.verification_level).capitalize())
            t, c, v = len(ctx.guild.text_channels), len(ctx.guild.categories), len(ctx.guild.voice_channels)
            tc, cc, vc = languages.gns(t, locale), languages.gns(c, locale), languages.gns(v, locale)
            embed.add_field(name=languages.gls("discord_server_channels", locale), value=languages.gls("discord_server_channels_data", locale, tc, cc, vc), inline=True)
            b, bl, bc = languages.gns(ctx.guild.premium_subscription_count, locale), languages.gns(ctx.guild.premium_tier, locale), \
                        languages.gns(len(ctx.guild.premium_subscribers), locale)
            embed.add_field(name=languages.gls("discord_server_boosts", locale), value=languages.gls("discord_server_boosts_data", locale, b, bl, bc), inline=True)
            ani = len([emote for emote in ctx.guild.emojis if emote.animated])
            total_emotes = len(ctx.guild.emojis)
            el = ctx.guild.emoji_limit
            na = total_emotes - ani
            n, a, e, t = languages.gns(na, locale), languages.gns(ani, locale), languages.gns(el, locale), languages.gns(total_emotes, locale)
            embed.add_field(name=languages.gls("discord_server_emotes", locale), value=languages.gls("discord_server_emotes_data", locale, n, a, e, t), inline=True)
            ca = ctx.guild.created_at
            ct, cd = languages.gts(ca, locale, short=False), languages.td_dt(ca, locale, suffix=True)
            embed.add_field(name=languages.gls("generic_created_at", locale), value=f"{ct}\n{cd}", inline=False)
            return await general.send(None, ctx.channel, embed=embed)

    @server.command(name="icon", aliases=["avatar"])
    async def server_icon(self, ctx: commands.Context):
        """ Get server icon """
        return await general.send(languages.gls("discord_server_icon", languages.gl(ctx), ctx.guild.name,
    ...
    async def server_banner(self, ctx: commands.Context):
        ...
        locale = languages.gl(ctx)
        if link:
            return await general.send(languages.gls("discord_server_banner", locale, ctx.guild.name, link), ctx.channel)
        else:
            return await general.send(languages.gls("discord_server_banner_none", locale, ctx.guild.name), ctx.channel)
    ...
    async def server_invite(self, ctx: commands.Context):
        ...
        locale = languages.gl(ctx)
        if link:
            return await general.send(languages.gls("discord_server_inv_bg", locale, ctx.guild.name, link), ctx.channel)
        else:
            return await general.send(languages.gls("discord_server_inv_bg_none", locale, ctx.guild.name), ctx.channel)
    ...
    async def server_bots(self, ctx: commands.Context):
        ...
        for i in range(len(bots)):
            m += f"[{languages.gns(i + 1, locale, 2, False)}] {bots[i]}\n"
        rl = len(m)
        send = languages.gls("discord_server_bots_data", locale, ctx.guild.name)
        if rl > 1900:
            async with ctx.typing():
                data = BytesIO(str(m).encode('utf-8'))
    ...
    async def remind_me(self, ctx: commands.Context, duration: str, *, reminder: str):
        ...
        if time.rd_is_above_5y(delta):
            return await general.send(languages.gls("util_reminders_limit", locale), ctx.channel)
            # return await general.send("You can't specify a time range above 5 years.", ctx.channel)
        expiry, error = time.add_time(delta)
        if error:
            return await general.send(languages.gls("util_reminders_error", locale), ctx.channel)
            # return await general.send(f"Failed to convert duration: {expiry}", ctx.channel)
        diff = languages.td_rd(delta, locale, accuracy=7, brief=False, suffix=True)
        when = languages.gts(expiry, locale, True, True, False, True, False)
        ...
        return await general.send(languages.gls("util_reminders_success", locale, ctx.author.name, diff, when), ctx.channel)
    ...
    async def reminders(self, ctx: commands.Context):
        ...
        if ctx.invoked_subcommand is None:
            ...
            if not reminders:
                return await general.send(languages.gls("util_reminders_none", locale, ctx.author.name), ctx.channel)
                # return await general.send(f"You have no reminders active at the moment, {ctx.author.name}.", ctx.channel)
            output = languages.gls("util_reminders_list", locale, ctx.author)
            ...
            for reminder in reminders:
                ...
                expires_on = languages.gts(expiry, locale, True, True, False, True, False)
                expires_in = languages.td_dt(expiry, locale, accuracy=3, brief=False, suffix=True)
                outputs.append(languages.gls("util_reminders_item", locale, _reminder, reminder["message"], reminder["entry_id"], expires_on, expires_in))
            ...
            try:
                ...
                return await general.send(languages.gls("util_reminders_dms", locale), ctx.channel)
    ...
    async def reminders_edit(self, ctx: commands.Context, reminder_id: int, *, args: str):
        ...
        if not reminder:
            return await general.send(languages.gls("util_reminders_edit_none", locale, reminder_id), ctx.channel)
        parser = arg_parser.Arguments()
        ...
        if args.time is not None:
            ...
            if len(_datetime) == 1:
                ...
                return await general.send(languages.gls("util_reminders_edit_time2", locale), ctx.channel)
            try:
                _expiry = datetime.strptime(f"{_date} {_time}", "%Y-%m-%d %H:%M:%S")
            except ValueError:
                return await general.send(languages.gls("util_reminders_edit_time", locale), ctx.channel)
        expiry = languages.gts(_expiry, locale, True, True, False, True, False)
        self.bot.db.execute("UPDATE temporary SET message=?, expiry=? WHERE entry_id=?", (_message, _expiry, reminder_id))
        return await general.send(languages.gls("util_reminders_edit", locale, reminder_id, _message, expiry), ctx.channel)
    ...
    async def reminders_delete(self, ctx: commands.Context, reminder_id: int):
        ...
        return await general.send(languages.gls("util_reminders_delete", locale, reminder_id), ctx.channel)
