...
from utils import http, argparser, emotes


async def image_gen(ctx, user: discord.Member, link, filename=None, extra_args=None):
...
class Images(commands.Cog):
    ...
    async def trash(self, ctx, *, user: discord.Member):
        ...
        if user == ctx.bot.user:
            return await ctx.send(f"How dare you call me trash! {emotes.AlexHeartBroken}")
        a1, a2 = [ctx.author.avatar_url, user.avatar_url]
        if user.id == 302851022790066185:
            a2, a1 = a1, a2
        bio = BytesIO(await http.get(
            f"https://api.alexflipnote.dev/trash?face={a1}&trash={a2}",
            res_method="read"))
        if bio is None:
            return await ctx.send("Something went wrong, couldn't generate image")
        return await ctx.send(file=discord.File(bio, filename=f"trash.png"))
