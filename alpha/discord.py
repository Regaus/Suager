...
from alpha.main import settings_template
...
class Discord(commands.Cog):
    ...
    async def settings(self, ctx):
        """ Server settings """
        if ctx.invoked_subcommand is None:
            data = self.db.fetchrow(f"SELECT * FROM data_{self.type} WHERE type=? AND id=?", ("settings", ctx.guild.id))
            if data:
    ...
    async def settings_upload(self, ctx):
        ...
        data = self.db.fetchrow(f"SELECT * FROM data_{self.type} WHERE type=? AND id=?", ("settings", ctx.guild.id))
        if data:
            res = self.db.execute(f"UPDATE data_{self.type} SET data=? WHERE type=? AND id=?",
                                  (stuff, "settings", ctx.guild.id))
        else:
            res = self.db.execute(f"INSERT INTO data_{self.type} VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    async def grant_custom_role(self, ctx, user: discord.Member, role: discord.Role):
        """ Grant custom role """
        if ctx.guild.id == 568148147457490954:
            already = self.db.fetchrow("SELECT * FROM custom_role WHERE uid=?", (user.id,))
            if not already:
                result = self.db.execute("INSERT INTO custom_role VALUES (?, ?)", (user.id, role.id))
                await user.add_roles(role, reason="Custom Role grant")
                return await ctx.send(f"Granted {role.name} to {user.name}: {result}")
            else:
                result = self.db.execute("UPDATE custom_role SET rid=? WHERE uid=?", (role.id, user.id))
