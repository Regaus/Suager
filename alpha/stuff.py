...
find = f'SELECT * FROM data_{main.version} WHERE id=? and type=?'  # Type is 'aqos'
insert = f'INSERT INTO data_{main.version} VALUES (?, ?, ?, ?, ?, ?, ?)'
# user.id, "aqos", data, False, user.name, user.discriminator, data['score']
update = f'UPDATE data_{main.version} SET data=?, usage=?, name=?, disc=?, extra=? WHERE id=? AND type=?'
